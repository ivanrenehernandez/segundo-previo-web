				...
				<div class="container-fluid">
					<div class="navbar-wrapper">
						...
						<c:if test="${votante != null}">
							<a class="navbar-brand" href="#pablo">Editar votante</a>
						</c:if>
						<c:if test="${votante == null}">
							<a class="navbar-brand" href="#pablo">Registrar votante</a>
						</c:if>
					</div>
					...
					<div class="col-md-8">
						<div class="card">
							<div class="card-header">
								<c:if test="${votante != null}">
									<h5 class="title">Editar votante</h5>
								</c:if>
								<c:if test="${votante == null}">
									<h5 class="title">Registrar votante</h5>
								</c:if>
							</div>
							<div class="card-body">
								<c:if test="${votante != null}">
									<form class="mx-auto w-full max-w-lg my-8 px-2"
										action="/ufps-elecciones/votantes/actualizar" method="post">
								</c:if>
								<c:if test="${votante == null}">
									<form class="mx-auto w-full max-w-lg my-8 px-2"
										action="/ufps-elecciones/votantes/registrar" method="post">
								</c:if>
								<c:if test="${votante != null}">
									...
									<div class="col-md-6 pr-1">
										<div class="form-group">
											<label>Nombres</label> <input type="text"
												...
												required value="<c:out value='${votante.nombre}'/>">
										</div>
									</div>
									<div class="col-md-6">
										<div class="form-group">
											<label>Email</label> <input type="text"
												class="form-control" placeholder="Correo electrónico" name="email"
												required value="<c:out value='${votante.email}'/>">
										</div>
									</div>
								</div>
								<div class="row">
									<div class="col-md-4">
										<div class="form-group">
											<label for="exampleInputEmail1">Documento</label> <input
												type="number" class="form-control" name="documento" placeholder="Documento" required
												value="<c:out value='${votante.documento}'/>">
										</div>
									</div>
									<div class="col-md-4 pr-1">
										<div class="form-group">
											<label for="exampleInputEmail1">Tipo Documento</label> <select
